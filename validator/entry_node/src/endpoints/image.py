...
import asyncio
from validator.entry_node import constants as ecst
...
async def make_non_stream_organic_query(
    ...
    while attempts_to_confirm < ecst.MAX_ATTEMPTS_TO_CONFIRM_JOB_SUBMISSION:
        if await _confirm_job_submission(pubsub, redis_db, job_id, organic_message):
            break
        attempts_to_confirm += 1
    if attempts_to_confirm == ecst.MAX_ATTEMPTS_TO_CONFIRM_JOB_SUBMISSION:
        logger.error(
            f"❌ Failed to confirm job submission for job {job_id} within {ecst.MAX_ATTEMPTS_TO_CONFIRM_JOB_SUBMISSION} attempts. Task: {task}"
        )
    ...
    while attempts_to_collect < ecst.MAX_ATTEMPTS_TO_COLLECT_RESULT:
        result = await _collect_results(pubsub, job_id, task, timeout)
        if result is not None:
            return result
        attempts_to_collect += 1
    logger.error(f"❌ No result for job {job_id} within {ecst.MAX_ATTEMPTS_TO_COLLECT_RESULT} attempts. Task: {task}")
    raise HTTPException(status_code=500, detail="Unable to process request - please try again!")
