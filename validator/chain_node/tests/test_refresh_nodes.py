...
from validator.chain_node.src.refresh_nodes import Config, get_and_store_nodes
...
class TestRefreshNodes(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_and_store_nodes(self, mock_fetch_nodes):
        ...
        mock_fetch_nodes.return_value = [
            Node(hotkey="test-hotkey1", coldkey="test-coldkey1", node_id=1, incentive=0.1, netuid=1, stake=30.0, trust=0.8, vtrust=0.7, ip="127.0.0.1", ip_type=4, port=1, protocol=4),
            Node(hotkey="test-hotkey2", coldkey="test-coldkey2", node_id=2, incentive=0.2, netuid=1, stake=20.0, trust=0.9, vtrust=0.8, ip="127.0.0.1", ip_type=4, port=2, protocol=4),
            Node(hotkey="test-vali", coldkey="test-vali-ck", node_id=3, incentive=0.3, netuid=1, stake=50.0, trust=1.0, vtrust=0.9, ip="127.0.0.1", ip_type=4, port=3, protocol=4)
        ]
