...
from validator.db.src.sql.nodes import get_nodes, get_vali_ss58_address
...
async def _calculate_metrics_and_quality_score(
    ...
    for reward_data in reward_datas:
        ...
        scoring_weight = (
            scst.SYNTHETIC_QUALITY_SCORE_WEIGHT
            if reward_data.synthetic_query
            else 1 - scst.SYNTHETIC_QUALITY_SCORE_WEIGHT
        )
        metrics[reward_data.node_hotkey] = metrics.get(reward_data.node_hotkey, []) + [
            reward_data.metric * scoring_weight
        ]
        quality_scores[reward_data.node_hotkey] = quality_scores.get(
            reward_data.node_hotkey, []
        ) + [reward_data.quality_score * scoring_weight]

    return metrics, quality_scores


async def _calculate_metric_bonuses(
    metrics: dict[str, list[float]]
) -> dict[str, float]:
    metric_scores = {
...
async def calculate_scores_for_settings_weights(
    ...
    for task, config in task_configs.items():
        ...
        combined_quality_scores, average_quality_scores, metric_bonuses = (
